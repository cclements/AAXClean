...
public class PresentationWindowContractTests
{
	...
	[TestMethod]
	public async Task UsacWithoutStss_FindsTheLastRealIndependenceFrameBeyondTwoSeconds()
	{
		byte[] sourceBytes = CreateAc4Source(
			movieTimescale: 1000,
			mediaTimescale: 48_000,
			frameDelta: 48_000,
			samples: [0x81, 2, 3, 4, 0x85, 6, 7, 8, 9, 10, 11]);
		using var source = new AAXClean.Mp4File(new MemoryStream(sourceBytes));
		...
		esds.ES_Descriptor.DecoderConfig.AudioSpecificConfig.AscBlob
			= Convert.FromHexString("F9464800");
		ChapterInfo chapters = new(TimeSpan.FromMilliseconds(8500));
		chapters.AddChapter("trim", TimeSpan.FromSeconds(1));
		using var output = new MemoryStream();
		...
		ChunkEntry firstChunk = new ChunkEntryList(converted.Moov.AudioTrack).First();
		converted.InputStream.Position = firstChunk.ChunkOffset;
		Assert.AreEqual(0x85, converted.InputStream.ReadByte(),
			"The output must begin at the real independence frame at 4 s, not a dependent frame in an arbitrary 2 s lookback.");
		CollectionAssert.AreEqual(
			new uint[] { 1 },
			converted.Moov.AudioTrack.Mdia.Minf.Stbl.Stss!.SampleNumbers);
		Assert.AreEqual(216_000L, converted.Moov.AudioTrack.Edts!.Elst!.SingleEdit!.Value.MediaTime);
		Assert.AreEqual(48_000L, converted.PresentedDurationSamples);
	}
	...
	[TestMethod]
	public async Task EmptyEditList_IsTreatedLikeNoPresentationEdit()
	{
		byte[] sourceBytes = CreateAc4Source(
			movieTimescale: 1000,
			mediaTimescale: 1000,
			frameDelta: 1000,
			samples: [1, 2, 3, 4],
			edits: Array.Empty<ElstBox.EditEntry>());
		using var source = new AAXClean.Mp4File(new MemoryStream(sourceBytes));
		using var output = new MemoryStream();

		Assert.AreEqual(0L, source.PresentationStartSample);
		Assert.AreEqual(4000L, source.PresentedDurationSamples);
	...
	[TestMethod]
	public async Task MovieTimescaleQuantization_DoesNotRejectAPlayableEditWindow()
	{
		byte[] sourceBytes = CreateAc4Source(
			movieTimescale: 1000,
			mediaTimescale: 44_100,
			frameDelta: 1024,
			samples: Enumerable.Repeat((byte)1, 21_081).ToArray(),
			edits: [new ElstBox.EditEntry(489_500, 0)]);
		using var source = new AAXClean.Mp4File(new MemoryStream(sourceBytes));
		using var output = new MemoryStream();

		Assert.AreEqual(21_586_944ul, source.Moov.AudioTrack.Mdia.Mdhd.Duration);
		Assert.AreEqual(21_586_950ul, ElstBox.ScaleDuration(
			489_500, source.Moov.Mvhd.Timescale, source.Moov.AudioTrack.Mdia.Mdhd.Timescale));

		await source.ConvertToMp4aAsync(output);

		Assert.IsNotEmpty(output.ToArray());
		using var converted = new AAXClean.Mp4File(new MemoryStream(output.ToArray()));
		Assert.AreEqual(21_586_944L, converted.PresentedDurationSamples);
	}
