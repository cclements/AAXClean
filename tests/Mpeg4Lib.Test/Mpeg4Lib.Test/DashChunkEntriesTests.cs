...
public class DashChunkEntriesTests
{
	...
	private static byte[] MakeFragment(
		uint sequence,
		uint? decodeTime,
		byte[] tfhd,
		byte[] trun,
		int mediaBytes,
		byte[]? additionalTrafBox = null)
	{
		byte[] mfhd = MakeBox("mfhd", UInt32sBE(0, sequence));
		var trafPayloads = new List<byte[]> { tfhd };
		if (decodeTime is uint value)
			trafPayloads.Add(MakeBox("tfdt", UInt32sBE(0, value)));
		if (additionalTrafBox is not null)
			trafPayloads.Add(additionalTrafBox);
		trafPayloads.Add(trun);
		byte[] traf = MakeBox("traf", trafPayloads.ToArray());
	...
	[TestMethod]
	public void LaterFragmentSeig_FailsBeforeYieldingItsChunk()
	{
		var samples = new[] { (100u, 1u, 0u) };
		byte[] first = MakeFragment(
			1, 0, MakeTfhd(), MakeTrun(samples, durations: true, sizes: true), mediaBytes: 1);
		byte[] seig = MakeBox(
			"sgpd",
			UInt32sBE(0),
			System.Text.Encoding.ASCII.GetBytes("seig"));
		byte[] second = MakeFragment(
			2, 100, MakeTfhd(), MakeTrun(samples, durations: true, sizes: true), mediaBytes: 1,
			additionalTrafBox: seig);
		var parsed = ParseFirstFragment(first, second);
		using var stream = parsed.Stream;
		var sidx = MakeSidx(1_000, 0, (first.Length + second.Length, 200));
		var entries = new DashChunkEntries(
			stream, 7, sidx, parsed.Moof, parsed.Mdat,
			minimumSample: 0, maximumSample: long.MaxValue,
			trackExtends: null, mediaTimescale: 1_000,
			rejectCencSampleGroups: true);
		using var enumerator = entries.GetEnumerator();
		...
		Assert.ThrowsExactly<NotSupportedException>(() => enumerator.MoveNext());
	}
	...
	[TestMethod]
	public void NewSidxReferenceWithoutTfdt_UsesItsExactIndexedStart()
	{
		var samples = new[] { (100u, 1u, 0u), (100u, 1u, 0u) };
		byte[] first = MakeFragment(
			1, decodeTime: null, MakeTfhd(), MakeTrun(samples, durations: true, sizes: true), mediaBytes: 2);
		byte[] second = MakeFragment(
			2, decodeTime: null, MakeTfhd(), MakeTrun(samples, durations: true, sizes: true), mediaBytes: 2);
		var parsed = ParseFirstFragment(first, second);
		using var stream = parsed.Stream;
		var sidx = MakeSidx(1_000, 100, (first.Length, 500), (second.Length, 500));
		var entries = new DashChunkEntries(
			stream, 7, sidx, parsed.Moof, parsed.Mdat,
			minimumSample: 0, maximumSample: long.MaxValue,
			trackExtends: null, mediaTimescale: 48_000);
		using var enumerator = entries.GetEnumerator();

		Assert.IsTrue(enumerator.MoveNext());
		ChunkEntry firstEntry = enumerator.Current;
		Assert.AreEqual(4_800L, firstEntry.FirstSample);
		stream.Position = firstEntry.ChunkOffset + firstEntry.ChunkSize;

		Assert.IsTrue(enumerator.MoveNext());
		Assert.AreEqual(28_800L, enumerator.Current.FirstSample);
	}

	[TestMethod]
	public void FragmentCrossingSidxReferenceBoundary_FailsClosed()
	{
		var samples = new[] { (100u, 1u, 0u), (100u, 1u, 0u) };
		byte[] fragment = MakeFragment(
			1, 0, MakeTfhd(), MakeTrun(samples, durations: true, sizes: true), mediaBytes: 2);
		var parsed = ParseFirstFragment(fragment);
		using var stream = parsed.Stream;
		var sidx = MakeSidx(1_000, 0, (fragment.Length - 1, 200), (1, 200));
		var entries = new DashChunkEntries(
			stream, 7, sidx, parsed.Moof, parsed.Mdat,
			minimumSample: 0, maximumSample: long.MaxValue,
			trackExtends: null, mediaTimescale: 1_000);
		...
		Assert.ThrowsExactly<InvalidDataException>(() => enumerator.MoveNext());
	}
}
