...
internal class DashFilter : AacValidateFilter
{
		...
		: base(audioIsUsac)
	{
		...
		if (PerSampleIvSize == 0)
		{
			if (trackEncryption.DefaultConstantIv.Length is not (8 or AesCtr.AES_BLOCK_SIZE))
				throw new InvalidDataException("A protected CENC track without per-sample IVs requires an 8- or 16-byte constant IV.");
			ConstantIv = (byte[])trackEncryption.DefaultConstantIv.Clone();
		}
		else if (PerSampleIvSize is not (8 or AesCtr.AES_BLOCK_SIZE))
		{
			throw new InvalidDataException($"Unsupported CENC per-sample IV size {PerSampleIvSize}; expected 8 or {AesCtr.AES_BLOCK_SIZE} bytes.");
		}
	...
	public override FrameEntry PerformFiltering(FrameEntry input)
	{
		...
		{
			if (AesCtr is null)
				throw new InvalidOperationException("A protected CENC track requires an initialized AES-CTR decryptor.");
			...
			else if (input.ExtraData is byte[] sampleIv)
			{
				if (sampleIv.Length != PerSampleIvSize)
					throw new InvalidDataException($"CENC sample IV is {sampleIv.Length} bytes, but tenc requires {PerSampleIvSize} bytes.");
				iv = sampleIv;
			}
			else
			{
				throw new InvalidDataException("A protected CENC sample has no usable per-sample or constant IV.");
			}
