...
public class ElstBox : FullBox
{
	...
	/// writes and honors. An empty list has no presentation mapping and returns null. Every
	/// unsupported non-empty form throws so a remux cannot silently discard a presentation
	/// mapping it does not understand.
	/// </summary>
	...
	/// <summary>
	/// Resolve a supported presentation edit against the available media. An edit-list
	/// duration is stored in whole movie-timescale ticks, so its media-timescale conversion
	/// may extend by less than one movie tick. Clamp only that quantization remainder; reject
	/// any substantive out-of-bounds window.
	/// </summary>
	public static long GetPresentedMediaDuration(
		EditEntry edit,
		long mediaDuration,
		uint movieTimescale,
		uint mediaTimescale)
	{
		if (edit.MediaTime > mediaDuration)
			throw new InvalidDataException(
				$"The presentation window starts at media sample {edit.MediaTime}, beyond mdhd duration {mediaDuration}.");

		long presentedDuration = checked((long)ScaleDuration(
			edit.SegmentDuration,
			movieTimescale,
			mediaTimescale));
		long availableDuration = mediaDuration - edit.MediaTime;
		if (presentedDuration <= availableDuration)
			return presentedDuration;

		BigInteger encodedMediaUnits = (BigInteger)edit.SegmentDuration * mediaTimescale;
		BigInteger availableMediaUnits = (BigInteger)availableDuration * movieTimescale;
		BigInteger overshoot = encodedMediaUnits - availableMediaUnits;
		if (overshoot > 0 && overshoot < mediaTimescale)
